...
| ppo-free-variables-review | 価値損失係数 0.5、Tanh、直交初期化は §7 の表になかった値をワーカーが補ったもの。G1 前に妥当性を見て固定する | G1 の学習を始める前 |
| gpu-memory-at-128-256 | 学習ループ込みの GPU メモリと身体ステップ/s を 128 と 256 環境で測り、並列数を決める（§10.3） | G1 の学習を始める前 |
| g1-training-run | `train_mlp.py --nworld 64 --total-steps 200000000` で G1 を回し、報酬係数と §3.2 の上限トルクを固定する。64 環境で約 6.4 時間 | 上の三つの後 |
...
| ppo-norm-consistency | PPO の更新で生の観測を最終の移動統計で正規化しており、収集時の正規化と僅かにずれる。正規化済み観測を保存する形を検討 | G1 の学習曲線が伸びないとき |
